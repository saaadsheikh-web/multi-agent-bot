...
"""
LONG BACKTEST — 60 days of 1H data, all 12 enabled agents × 9 timeframes × 5 coins
Uses the bot's own BloFin client and agent analyze() methods.

Runs at 1H bar resolution (1440 bars) for speed, resamples to all TFs.
Each agent checks at its TF bar close only.

Results saved to long_backtest_results.json
"""
...
# ALL 12 enabled agents (including macd_cross)
AGENT_SPECS = [
...
# 60 days = 1440 1H bars
MAX_BARS = 1440
WARMUP = 240  # 10 days warmup

MIN_CONFIDENCE = 6
MAX_HOLD_BARS = 720  # max 30 days hold
...
def resample_to_tf(df, chunk_size):
    """Resample OHLCV by chunking N consecutive 1H bars."""
    df = df.copy()
...
def fetch_data(symbol):
    """Fetch 1440 1H bars (~60 days), resample to all TFs."""
    df_1h = bf.candles(symbol, "1H", MAX_BARS)
    if df_1h.empty:
...
def build_context(symbol, all_data, idx_1h):
    """Build MarketContext at index idx_1h of 1H data."""
...
def simulate_trade(signal, df_future_1h, entry_idx):
    """Simulate trade on forward 1H data with SL/TP."""
    side = signal.side
    meta = signal.metadata or {}
    entry_price = float(df_future_1h.iloc[0]["open"])

    # SL/TP from signal metadata or profile default
...
def backtest_one(agent_cls, agent_name, tf, symbol, all_data):
    """Backtest one agent on one timeframe for one symbol using 1H data."""
    agent = agent_cls()
    df_1h = all_data["1H"]
    n_bars = len(df_1h)
    stride = TF_CHUNKS[tf]
    ...
    last_sig = None
    last_sig_idx = -9999
    min_gap = stride * 48  # at least 2 days between same signal
...
def main():
    print("=" * 70)
    print(f"LONG BACKTEST: {len(AGENT_SPECS)} agents × {len(TIMEFRAMES)} TFs × {len(SYMBOLS)} symbols")
    print(f"  60 days of 1H data (1440 bars), {WARMUP} bar warmup")
    ...
    for symbol in SYMBOLS:
        ...
        print(f"  ✓ {len(dfh)} 1H bars: "
              f"{datetime.fromtimestamp(dfh['ts'].iloc[0]/1000).strftime('%m-%d %H:%M')} → "
              f"{datetime.fromtimestamp(dfh['ts'].iloc[-1]/1000).strftime('%m-%d %H:%M')}")
        ...
        for agent_name, agent_cls in AGENT_SPECS:
            ...
            sys.stdout.flush()
            for tf in TIMEFRAMES:
                # Skip TFs that have fewer than 5 bars after warmup
    ...
    out_path = "/Users/saad/multi_agent_bot/long_backtest_results.json"
