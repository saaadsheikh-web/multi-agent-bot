...
"""
BLOFIN 1-YEAR BACKTEST V2 — 5 live agents × 9 TFs × 5 coins.
Uses BloFin 1H data (paginated ~365 days), resamples to HTFs.

Only tests agents that survived the TV backtest v2 filter:
  fib_confluence, daily_breakout_24h, daily_breakout_12h,
  daily_breakout_48h, livermore_pivot
"""
...
# Only the 5 agents that passed TV backtest v2
AGENT_SPECS = [
...
def simulate_trade(signal, df_future_1h, entry_idx):
    ...
    for i in range(1, len(df_future_1h)):
        ...
        if side == "long":
            if high >= tp_price:
                pnl_pct = (tp_price / entry_price - 1) * 100 - 0.08  # fees
...
def main():
    print("=" * 70)
    print(f"BLOFIN 1-YEAR BACKTEST V2: {len(AGENT_SPECS)} agents × {len(TIMEFRAMES)} TFs × {len(SYMBOLS)} symbols")
    ...
    out_path = "/Users/saad/multi_agent_bot/one_year_backtest_v2_results.json"
