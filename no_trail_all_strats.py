#!/usr/bin/env python3
"""TEST ALL STRATEGIES — no trail, all TFs. Find what actually works."""
import os, json, pandas as pd
import numpy as np
from collections import defaultdict

CACHE = os.path.expanduser("~/multi_agent_bot/backtest_data")
OUT = os.path.expanduser("~/multi_agent_bot/NO_TRAIL_ALL_STRATS.json")
...
def simulate(trades, d, tr_act=1.0, tr_dist=0):
...
# ===== ALL STRATEGIES =====
def breakout_gen(d, lb, sl_pct, mv):
    ...
    for i in range(lb+30, d.n):
        ...
        if last>rh and last>bar_o:
            sr=bar_o+bar_r*0.5; sf=last*(1-sl_pct)
...
def donchian(d):
    ...
    for i in range(50, d.n):
        ...
        if p>hn: tr.append({"idx":i,"side":"long","entry":p,"tp":p*99,"sl":p-atr[i]*2.0})
        elif p<ln: tr.append({"idx":i,"side":"short","entry":p,"tp":p*0.01,"sl":p+atr[i]*2.0})
    return tr
...
def vol_cap(d):
    ...
    for i in range(30, n):
        ...
        if pct<-3 and vr>4 and r<30: tr.append({"idx":i,"side":"long","entry":c[i],"tp":c[i]*99,"sl":c[i]*0.985})
...
def asian_pump(d):
    ...
    for i in range(30, n):
        ...
        if pct>0: tr.append({"idx":i,"side":"long","entry":c[i],"tp":c[i]*99,"sl":c[i]*0.975})
        else: tr.append({"idx":i,"side":"short","entry":c[i],"tp":c[i]*0.01,"sl":c[i]*1.025})
    return tr

STRATEGIES = {
    "B4h":   lambda d: breakout_gen(d, 4, 0.012, 1.3),
    "B8h":   lambda d: breakout_gen(d, 8, 0.015, 1.4),
    "B12h":  lambda d: breakout_gen(d, 12, 0.018, 1.5),
    "B24h":  lambda d: breakout_gen(d, 24, 0.025, 1.8),
    "B48h":  lambda d: breakout_gen(d, 48, 0.030, 1.9),
    "B168h": lambda d: breakout_gen(d, 168, 0.040, 2.0),
    "fib_ext": fib_ext,
    "donchian": donchian,
    "trend_pb": trend_pb,
    "vol_cap": vol_cap,
...
print(f"No-trail backtest: {len(SYMBOLS)} coins x {len(TFS)} TFs x {len(STRATEGIES)} strats")
result = {"config":{"no_trail":True,"tfs":TFS},"ranking":[]}
...
for sname, sfunc in STRATEGIES.items():
    for tf in TFS:
        ...
        for sym in SYMBOLS:
            ...
            if st["n"]>0:
                st2=dict(st); st2["symbol"]=sym; st2["tf"]=tf; st2["strategy"]=sname
                sym_res.append(st2)
        ...
        best=max(sym_res, key=lambda x:x["total_r"])
        worst=min(sym_res, key=lambda x:x["total_r"])
...
print(f"\n{'='*90}")
print(f"  NO-TRAIL BACKTEST RANKING")
print(f"{'='*90}")
print(f"{'Rank':5s} {'Strat':10s} {'TF':5s} {'Trades':8s} {'WR':7s} {'Total$':10s} {'TotalR':10s} {'Prof':5s} {'BestSym':10s}")
print("-"*70)
