...
class TrendPullbackAgent(Agent):
    ...
    name = "trend_pullback"
    enabled = False  # 2026-06-03: KILLED — 1yr backtest: -21% to -47%, 63% WR but trail too tight vs SL
    paper_only = False   # validate before live
...
class DailyBreakout4hAgent(_DailyBreakoutBase):
    ...
    # 2026-06-03: SCALED — 1yr backtest: 10,058 trades, 74.5% WR, +0.622 ExpR (#1 agent)
    LOOKBACK_BARS = 4
    MIN_SL_PCT = 0.012
...
class MACDCrossAgent(Agent):
    notional_multiplier = 0.06  # 2026-06-07: 2x — best agent: 31t, 71% WR, +$32.37
    """2026-06-04: 5m PROVEN — +259.6% on 46 coins. MACD cross with crypto-optimized settings (24/52/18) + volume + ATR SL.
    Research: crypto MACD needs faster settings than stocks. Standard 12/26/9
    is too slow for crypto's 24/7 volatility."""
    name = "macd_cross"
    enabled = True  # 2026-06-04: PROVEN WINNER — 30t, 72% WR, +$17.41 — only macd_cross + fib_bounce survive
    paper_only = False
    profile = "macd_cross"
    valid_regimes = ["TRENDING", "VOLATILE"]
...
async def _tv_handle(request: "aiohttp.web.Request", state) -> "aiohttp.web.Response":
    ...
    tk = next((t for t in tickers if t.get("instId") == symbol), None)
...
async def run(paper: bool = False, once: bool = False):
    ...
    DEAD_AGENT_PURGE = {
        # 2026-06-04: KILLED — losing agents. Only winners survive.
        # 2026-06-05: RESTORED winners, then RE-KILLED daily_breakout_24h (4t -$10.90)
        # and daily_breakout_4h (3t -$0.80) based on live learning data.
        "asian_pump", "asian_session",
        "atr_momentum",
        "candlestick",
        ...
        "trend_pullback", "tv_fibonacci",  # LIVE LOSER: 2t, -$10.09
        "us_open", "utbot_mtf", "utbot_v3",
        "volume_capitulation", "volume_profile",
